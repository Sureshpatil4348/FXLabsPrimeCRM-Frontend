...
export async function POST(req: Request) {
  try {
    ...
    const url = process.env.SUPABASE_RESET_ADMIN_PASSWORD_FUNCTION_URL

    if (!url) {
      return NextResponse.json({ error: "Server not configured: missing SUPABASE_RESET_ADMIN_PASSWORD_FUNCTION_URL" }, { status: 500 })
    }
    ...
    if (!response.ok && response.status !== 207) {
      return NextResponse.json({ error: data.error || "Failed to reset admin password" }, { status: response.status })
    }
